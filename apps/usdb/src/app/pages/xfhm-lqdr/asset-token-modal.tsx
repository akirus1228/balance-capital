        ...
        <Box className={style['asset-token-modal']} padding="20px">
          ...
          </Box>
          {
            props?.assetTokens.map((token: AssetToken, index: number) => {
              return (index !==0 && <Box display="flex" justifyContent="space-between" className="cursor-pointer" width="100%" alignItems="center" mb="20px" key={`${index}-${token.name}`} onClick={() => props.onClose(token)}>
