import { minutesAgo } from './date-time';
...
export class NodeHelper {
  ...
  static _emptyInvalidNodesList() {
    ...
    if (
      ...
      Object.keys(NodeHelper.currentRemovedNodes).length ===
        EnvHelper.getAPIUris().length
    ) {
  ...
  static logBadConnectionWithTimer(providerUrl: string) {
    ...
    currentConnectionStats = NodeHelper._updateConnectionStatsForProvider(
      currentConnectionStats
    );

    if (
      currentConnectionStats.failedConnectionCount >
      NodeHelper._maxFailedConnections
    ) {
      ...
      NodeHelper._storage.setItem(
        providerKey,
        JSON.stringify(currentConnectionStats)
      );
    }
  }
