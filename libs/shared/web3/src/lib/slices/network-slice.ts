...
export const loadNetworkDetails = createAsyncThunk(
  ...
  async ({ networkId }: IBaseAsyncThunk, { dispatch }): Promise<INetworkDetails> => {
    const provider = await chains[networkId].provider;
		...
		const [marketPrice, daoSfhmBalance, epoch, originalStakingCircSupply, totalSupply, circSupply, currentIndex] = await Promise.all([
      ...
      0,
		...
		const secondsPerEpoch = networks[networkId].blocktime * networks[networkId].epochInterval
