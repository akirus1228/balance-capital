...
export class FallbackProvider extends Provider {
  ...
  async call(transaction: Deferrable<TransactionRequest>, blockTag?: BlockTag | Promise<BlockTag>): Promise<string> {
    return String(this._call(transaction, blockTag));
  }


  private async _call(transaction: Deferrable<TransactionRequest>, blockTag?: BlockTag | Promise<BlockTag>, attempts = 0): Promise<unknown> {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < Math.min(this.nodes.length, this.useTopN); i++) {
