...
import { DebugHelper } from "@fantohm/helpers";
...
export class MulticallProvider extends Provider {
  ...
  constructor(networkName: string, provider: Provider, multicallAddress: string) {
    ...
    this.multicallContract = new ethers.Contract(multicallAddress, MulticallContract, provider);
  ...
  private flushQueue(): void {
    if (this.requestQueue == null || this.requestQueue.length === 0) {
      ...
      this.requestQueue = null;
      if (DebugHelper.isActive('enable-tracing')) console.log(`${this.networkName}: Requests to flush: ${toFlush.length}`);
      const aggregatedRequest = toFlush.map(request => request.transaction).map(transaction => [transaction.to, transaction.data]);
      const resolves = toFlush.map(request => request.resolve);
      const rejects = toFlush.map(request => request.reject);
      this.multicallContract['tryAggregate'](false, aggregatedRequest).then((results: any) => {
  ...
  call(transaction: ethers.utils.Deferrable<TransactionRequest>, blockTag?: BlockTag | Promise<BlockTag>): Promise<string> {
    return this.disabled ? this.provider.call(transaction, blockTag) : new Promise((resolve, reject) => {
